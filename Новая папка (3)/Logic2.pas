...
begin
  ...
  begin
    ...
    begin
      ...
      if startStation^.next^.data = stat2 then
      begin
        Result := true;
        exit;
      end;
...
function GetTime(var pnt1, pnt2 : PTStationList) : double;
const
...
begin
  if StationEnableOnBus(pnt1, pnt2) then
    answ := GetDistance(pnt1^.X, pnt1^.Y, pnt2^.X, pnt2^.Y) / BUS_SPEED
  else if StationEnableOnTram(pnt1, pnt2) then
    answ := GetDistance(pnt1^.X, pnt1^.Y, pnt2^.X, pnt2^.Y) / TRAM_SPEED
  else if StationEnableOnMetro(pnt1, pnt2) then
    answ := GetDistance(pnt1^.X, pnt1^.Y, pnt2^.X, pnt2^.Y) / METRO_SPEED
  else if CheckDistance(pnt1, pnt2) then
    answ := GetDistance(pnt1^.X, pnt1^.Y, pnt2^.X, pnt2^.Y) / ON_FOOT_SPEED
  else answ := 0;
  Result := answ;
end;
  ...
  begin
    ...
    begin
      ...
      (connectMatrix[prev^.num - 1, i].time <> 0) then
        ...
        (connectMatrix[prev^.num - 1, i].distance > distance + connectMatrix[prev^.num - 1, i].time) then
        begin
